...
fn gen_deposit(date: NaiveDate) -> impl Strategy<Value = Transaction> {
    let currencies = all_currencies();
    prop::sample::select(currencies).prop_flat_map(move |currency| {
        (100u32..10000u32).prop_map(move |cents| {
            let amount = Money::new(Decimal::new(cents as i64, 2), currency);
            make_tx(TransactionKind::deposit(amount).unwrap(), date)
        })
    })
}

fn gen_withdrawal(date: NaiveDate) -> impl Strategy<Value = Transaction> {
    let currencies = all_currencies();
    prop::sample::select(currencies).prop_flat_map(move |currency| {
        (100u32..10000u32).prop_map(move |cents| {
            let amount = Money::new(Decimal::new(cents as i64, 2), currency);
            make_tx(TransactionKind::withdrawal(amount).unwrap(), date)
        })
    })
}

fn gen_fee(date: NaiveDate) -> impl Strategy<Value = Transaction> {
    let currencies = all_currencies();
    prop::sample::select(currencies).prop_flat_map(move |currency| {
        (0u32..10000u32).prop_map(move |cents| {
            let amount = Money::new(Decimal::new(cents as i64, 2), currency);
            make_tx(TransactionKind::fee(amount, None).unwrap(), date)
        })
    })
}

fn gen_buy(
    universe: &[(InstrumentId, Currency)],
    date: NaiveDate,
) -> impl Strategy<Value = Transaction> {
    let universe = universe.to_vec();
    prop::sample::select(universe).prop_flat_map(move |(inst, currency)| {
        (1u32..100u32, 100u32..10000u32, 0u32..500u32).prop_map(
            move |(qty, price_cents, fee_cents)| {
                let price = Money::new(Decimal::new(price_cents as i64, 2), currency);
                let fees = Money::new(Decimal::new(fee_cents as i64, 2), currency);
                make_tx(
                    TransactionKind::buy(inst, Decimal::from(qty), price, fees, None).unwrap(),
                    date,
                )
            },
        )
    ...
    prop::sample::select(universe).prop_flat_map(move |(inst, currency)| {
        (1u32..100u32, 100u32..10000u32, 0u32..500u32).prop_map(
            move |(qty, price_cents, fee_cents)| {
                ...
                let fees = Money::new(Decimal::new(fee_cents as i64, 2), currency);
                make_tx(
                    TransactionKind::sell(inst, Decimal::from(qty), price, fees, None).unwrap(),
                    date,
                )
            },
        )
...
fn gen_dividend(
    ...
    prop::sample::select(universe).prop_flat_map(move |(inst, currency)| {
        (100u32..10000u32).prop_map(move |cents| {
            let amount = Money::new(Decimal::new(cents as i64, 2), currency);
            make_tx(TransactionKind::dividend(inst, amount, None).unwrap(), date)
        })
    })
}
...
fn gen_split(
    ...
    let universe = universe.to_vec();
    prop::sample::select(universe).prop_flat_map(move |(inst, _currency)| {
        prop_oneof![Just(2u32), Just(4u32), Just(5u32), Just(8u32), Just(10u32)].prop_map(
            move |ratio| {
                make_tx(
                    TransactionKind::CorporateAction(
                        CorporateAction::split(inst, Decimal::from(ratio)).unwrap(),
                    ),
                    date,
                )
            },
...
fn gen_reverse_split(
    ...
    prop::sample::select(universe).prop_flat_map(move |(inst, _currency)| {
        prop_oneof![Just(2u32), Just(4u32), Just(5u32), Just(8u32), Just(10u32)].prop_map(
            move |denom| {
                let ratio = Decimal::ONE / Decimal::from(denom);
                make_tx(
                    TransactionKind::CorporateAction(
                        CorporateAction::reverse_split(inst, ratio).unwrap(),
                    ),
                    date,
                )
            },
